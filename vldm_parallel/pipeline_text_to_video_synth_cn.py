...
class CNTextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    r"""
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            untruncated_ids = self.tokenizer(text=prompt, padding='max_length', max_length=model_max_length, truncation=True, return_tensors="pt").input_ids

            if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not torch.equal(
               text_input_ids, untruncated_ids
            ):
               removed_text = self.tokenizer.batch_decode(
                   untruncated_ids[:, model_max_length - 1 : -1]
               )
               logger.warning(
            ...
            logger.warning(f'prompt_embeds: {prompt_embeds.size()}')
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            logger.warning(f'negative_prompt_embeds: {negative_prompt_embeds.size()}')
